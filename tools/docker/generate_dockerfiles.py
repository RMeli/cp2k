...
def spack_toolchain(variant: str) -> str:
    if variant == "cpu":
        output = rf"""
FROM ubuntu:22.04 as builder
"""
        output +=  spack_toolchain_core() + rf"""
ENV SPEC_OPENBLAS="cp2k@master%gcc build_system=cmake +enable_regtests +cosma +mpi +openmp ^openblas+fortran ^cosma+scalapack+shared ^dbcsr+mpi+shared"
""" + spack_toolchain_end()

    if variant == "cuda":
        output = rf"""
nvidia/cuda:12.3.0-devel-ubuntu22.04
        """ + spack_toolchain_core() + rf"""
